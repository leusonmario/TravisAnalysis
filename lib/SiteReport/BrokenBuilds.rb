  ...
  def getAllConflicts(files, projectNames)
    ...
    files.each do |file|
      ...
      CSV.foreach(file.to_s.gsub("./","").gsub("\n","")) do |row|
        if (row[8] == "true" and ((row[3] == "failed" or row[3] == "[\"failed\"]" or row[3] == "[\"passed\"]" or row[3] == "passed") and (row[5] == "failed" or row[5] == "[\"failed\"]" or row[5] == "[\"passed\"]" or row[5] == "passed") and row[21] == "false"))
