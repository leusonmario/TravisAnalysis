...
class ConflictCategoryErroredAll
					...
					build.jobs[indexJob].log.body do |bodyJob|
						if (bodyJob != nil)	
							...
							begin
								...
								print "TOO MUCH TIME TO ACCESS BUILD LOG"
							end
						end
					end
				end
	...
	def findConflictCauseLocalID(build, pathProject)
		localMalformedExp = 0
		stringCompError = " COMPILATION ERROR :"
		stringNotFindType = "not find: type"
		stringNoConvert = "cannot be converted to"
	...
	def getLogsBuild(buildId, projectPath)
		Dir.chdir projectPath
		logs = []

		begin
			infoBuild = %x(travis show #{buildId})
