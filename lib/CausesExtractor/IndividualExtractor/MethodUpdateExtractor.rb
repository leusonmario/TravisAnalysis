		...
		begin
			...
			if (buildLog[/\[ERROR\][ \t\r\n\f]*[a-zA-Z0-9\/\-\.\:\[\]\,\(\) ]* cannot be applied to [a-zA-Z0-9\/\-\.\:\[\]\,]*/])
				...
				while (count < changedClasses.size)
					...
					methodName = ""
					line = lines[count].to_s.match(/[0-9]*\,[0-9]*/)[0]
					if (callClassFile.include? " ")
							...
							filesInformation.push(["methodParameterListSize", callClassFile.split(',').last.to_s.gsub("\]",'').to_s.gsub(' ',''), callClassFile.split(',').first.to_s.gsub("\[",'').to_s.gsub(' ',''), changedClass, "method", line])
						else
								methodName = aux.split('] ').last.match(/[a-zA-Z]*/)
							puts "\n"
							print "\n #{methodName.size}"
							if (methodName[0] == "")
								methodName = aux.to_s.match(/ [a-zA-Z0-9]*/)[0].split(" ").last
							end
