...
class CopyFixCommit
  ...
  def createCopyProject(fixCommit, brokenCommit)
    ...
    invalidFiles = %x(find #{@pathCopyFix} -type f -regextype posix-extended -iregex '.*\.(sh|md|yaml|yml|conf|scala|properties|less|txt|gitignore|sql|html)$' -delete)
    ...
    invalidFiles = %x(find #{@brokenCopyBroken} -type f -regextype posix-extended -iregex '.*\.(sh|md|yaml|yml|conf|scala|properties|less|txt|gitignore|sql|html)$' -delete)
