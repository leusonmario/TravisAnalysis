class CopyProjectDirectories
	...
	def createCopyProject(mergeCommit, parents, pathProject)
		...
		invalidFiles = %x(find #{copyBranch[4]} -type f -regextype posix-extended -iregex '.*\.(sh|vm|md|yaml|yml|conf|scala|properties|less|txt|gitignore|sql|html)$' -delete)
		...
		while(index < parents.size)
			...
			invalidFiles = %x(find #{copyBranch[index+2]} -type f -regextype posix-extended -iregex '.*\.(sh|vm|md|yaml|yml|conf|scala|properties|less|txt|gitignore|sql|html)$' -delete)
