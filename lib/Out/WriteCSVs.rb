	...
	def printConflictBuild(build, buildOne, buildTwo, state, projectName, effort)
		...
		if (File.exists?("Errored"+projectName+".csv"))
			CSV.open("Errored"+projectName+".csv", "a+") do |csv|
				if (effort != nil)
					csv << [build, buildOne, buildTwo, state[0], state[2], state[1][0], state[1][1], state[1][2], effort[0], effort[1], effort[2], effort[3], effort[4], effort[5], effort[6]]
				else
			...
			CSV.open("Errored"+projectName+".csv", "ab") do |csv|
				csv << ["BuildID", "BuildParentOne", "BuildParentTwo", "MessageState", "NumberOccurrences", "ConflictingContributions", "AllColaborationsIntgrated", "BrokenBuild", "FixBuildID", "FixStatus", "Effort", "NumberBuildsPerformed", "SameAuthor", "SameCommiter", "BestCase"]
				if (effort != nil)
					csv << [build, buildOne, buildTwo, state[0], state[2], state[1][0], state[1][1], state[1][2], effort[0], effort[1], effort[2], effort[3], effort[4], effort[5], effort[6]]
				else
					csv << [build, buildOne, buildTwo, state[0], state[2], state[1][0], state[1][1], state[1][2], "", "", "", "", "", "", ""]
